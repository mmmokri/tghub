...
import '../../styles/admin/AdminCabinetPage.css';

interface AdminCabinetPageProps {
...
const AdminCabinetPage: React.FC<AdminCabinetPageProps> = ({ 
  onGoToNotifications, 
  onGoToEditProfile, 
  onLogout,
  onGoToCabinet,
  ...
  // Заглушки для групп
  const groups = [
    { id: 1, name: 'Группа #1', status: 'Текущая', statusColor: '#4CAF50' },
    { id: 2, name: 'Группа #2', status: 'Завершено', statusColor: '#FF4B4B' },
  ];

  return (
    <div className="admin-cabinet-container" style={containerStyle}>
      ...
      <main className="cabinet-content">
        ...
        <section className="profile-card" style={cardStyle}>
          ...
          <div className="profile-info">
            ...
            <div className="profile-tags">
              <span className="tag" style={tagStyle}>PHP</span>
              <span className="tag" style={tagStyle}>JS</span>
            </div>
          </div>
        </section>
        ...
        <section className="cabinet-section">
          <h3>Задачи для студентов</h3>
          <div className="groups-container" style={cardStyle}>
            {groups.map(group => (
              <div key={group.id} className="group-item">
                <div className="group-info">
                  <span className="group-name">{group.name}</span>
                  <span className="group-status" style={{ color: group.statusColor }}>({group.status})</span>
                </div>
                <div className="group-actions">
                  <button 
                    className="btn-small" 
                    onClick={onAddTask}
                    style={{ backgroundColor: '#2C353F', color: '#FFFFFF' }}
                  >
                    Добавить
                  </button>
                  <button 
        ...
        <div className="cabinet-actions">
          ...
          <button 
            ...
            style={{ borderColor: '#FF4B4B', color: '#FF4B4B', backgroundColor: 'transparent' }}
          >
            Выйти
          </button>
        </div>
