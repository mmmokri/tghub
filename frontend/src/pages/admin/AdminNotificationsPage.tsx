...
import '../../styles/admin/AdminNotificationsPage.css';
...
const AdminNotificationsPage: React.FC<AdminNotificationsPageProps> = ({ 
  ...
  const notifications = [
    {
      ...
      message: 'Студент Иванов Иван принял приглашение на собеседование🎉',
      address: 'г. Чайковский, ул Приморский бульвар, д 32/1, офис 312',
      dateTime: 'Пятница, 20 февраля, 14:00',
    }
  ];
  ...
  const tagStyle = { borderColor: theme.accent, color: theme.accent };

  return (
    <div className="admin-notifications-container" style={containerStyle}>
      ...
      <main className="notifications-content">
        {notifications.map(notification => (
          <section key={notification.id} className="notification-card" style={cardStyle}>
            <div className="notification-top">
              <div className="avatar-placeholder"></div>
              <div className="type-tag" style={tagStyle}>
                {notification.type}
              </div>
            </div>
            <div className="notification-body">
              <p className="main-message">{notification.message}</p>
              <div className="details-section" style={hintStyle}>
                <p>Собеседование состоится по адресу:</p>
                <p>{notification.address}</p>
              </div>
              <p className="date-time">{notification.dateTime}</p>
            </div>
          </section>
        ))}
      </main>
