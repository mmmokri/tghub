...
import '../../styles/admin/AdminTasksPage.css';
...
const AdminTasksPage: React.FC<AdminTasksPageProps> = ({ 
  onBack, 
  onGoToCabinet, 
  onGoToGrades,
  onEditTask
  ...
  const tasks = [
    {
      id: 1,
      title: 'Программа на JS',
      description: 'Создайте программу',
    },
    {
      id: 2,
      title: 'Верстка макета сайта кофейни',
      description: 'Сверстайте макет',
    }
  ];
  ...
  return (
    <div className="admin-tasks-container" style={containerStyle}>
      ...
      <main className="tasks-content">
        {tasks.map(task => (
          <section key={task.id} className="task-card" style={cardStyle}>
            <div className="task-info">
              <h2>{task.title}</h2>
              <p style={hintStyle}>{task.description}</p>
            </div>
